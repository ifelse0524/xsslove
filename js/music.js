...
function musicPlaySet(){
    ...
    stateSet.onclick = function(){
        // 点击暂停的时候
        if(bof.getAttribute("src",2) == "img/music_3.svg"){

            bof.setAttribute("src","img/music_4.svg");

        }else{

            bof.setAttribute("src","img/music_3.svg");
...
// 上一个曲目和下一个曲目方法
let musicList = ["LostMemory","You","Keep You Mine","Over Again","Lullaby","爱的魔法","缺氧"];
function musicJump(){
    for (let i = 0; i < musicList.length; i++) {
        musicLast.onclick = function(){
            musicInfoShow.innerText = musicList[i];
            musicOBJ.src = "music/" + musicList[i] + ".mp3";
            musicBOXbc.src = "img/" + musicList[i] + ".jpg";
            musicHead.src = "img/" + musicList[i] + ".jpg";
            nowtime = 0;
            console.log(i);
            i--;
            if(i < 0 || i > 7){
